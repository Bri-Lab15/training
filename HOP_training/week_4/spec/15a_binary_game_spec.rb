...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('a', '0', '4')
      end
...
context 'when user inputs an incorrect value once, then a valid input' do
  before do
    allow(game_input).to receive(:gets).and_return('d', '5')
  end
...
context 'when given a valid input as argument' do
  it 'returns valid input' do
    expect(game_check.verify_input('5')).to eq(5)
  end
end

context 'when given invalid input as argument' do
  it 'returns nil' do
    expect(game_check.verify_input('z')).to be_nil
    expect(game_check.verify_input('0')).to be_nil
    expect(game_check.verify_input('11')).to be_nil
  end
end
...
context 'when game minimum and maximum is 100 and 600' do
  subject(:game_range) { described_class.new(100, 600) }

  it 'returns 9' do
    expect(game_range.maximum_guesses).to eq(9)
  end
end
...
before do
  allow(binary_search_double).to receive(:make_guess).and_return(5)
...
it 'increases guess_count by one' do
  expect { game_turn.display_turn_order(binary_search_double) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
end

it 'sends make_guess' do
  expect(binary_search_double).to receive(:make_guess)
  game_turn.display_turn_order(binary_search_double)
end

it 'sends update_range' do
  expect(binary_search_double).to receive(:update_range)
  game_turn.display_turn_order(binary_search_double)
end
end
