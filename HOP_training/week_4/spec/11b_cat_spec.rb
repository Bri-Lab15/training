...
describe Cat do
  subject(:whiskers) { described_class.new('Whiskers', 'Siamese', 'cream') }

  # Use the second shared example to test that cat responds to talk
  context 'when testing the talk method' do
    include_examples 'talk method', 'meow'
  end

  it 'is not hungry' do
    expect(whiskers.hungry?).to eq(false)
  end

  it 'is hiding' do
    expect(whiskers.hiding?).to eq(true)
  end
end
