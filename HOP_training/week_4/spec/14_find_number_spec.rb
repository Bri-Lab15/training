...
describe FindNumber do
  # ASSIGNMENT: METHOD #1 - #make_guess
  describe '#make_guess' do
  ...
  # ASSIGNMENT: METHOD #2 - #game_over?
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      let(:number_double) { double('random_number', value: 5) }
      subject(:game) { described_class.new(0, 9, number_double, 5) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_double) { double('random_number', value: 7) }
      subject(:game) { described_class.new(0, 9, number_double, 5) }
  ...
  # ASSIGNMENT: METHOD #3 - #update_range
  describe '#update_range' do
    let(:number_range) { double('random_number', value: 8) }
    ...
    context 'edge case: guess is 7, min=5, max=8' do
      subject(:edge_case_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to 8' do
      ...
      it 'does not update max' do
        edge_case_game.update_range
        expect(edge_case_game.max).to eq(8)
      end
    end
  end
end
