...
describe Dog do
  subject(:buddy) { described_class.new('Buddy', 'Labrador', 'golden') }

  # Use the second shared example to test that dog responds to talk
  context 'when testing the talk method' do
    include_examples 'talk method', 'WOOF!'
  end

  it 'is not barking' do
    expect(buddy.barking?).to eq(false)
  end

  it 'is sleeping' do
    expect(buddy.sleeping?).to eq(true)
  end
end
