...
describe CaesarBreaker do
  ...
  # =========================
  describe '#create_decrypted_messages' do
    it 'calls translate 26 times' do
  ...
  # =========================
  describe '#save_decrypted_messages' do
    # ---- ASSIGNMENT #2: directory does NOT exist ----
    context 'when the directory does not exist' do
      before do
        allow(Dir).to receive(:exist?).and_return(false)
        allow(Dir).to receive(:mkdir)
    ...
    context 'when the directory exists' do
      ...
      it 'does not create the directory' do
        phrase.save_decrypted_messages
        expect(Dir).not_to have_received(:mkdir)
      end
    ...
    # ---- Test: error occurs while saving ----
  ...
  # =========================
  describe '#save_to_yaml' do
    it 'calls YAML.dump with the messages' do
      messages = [ 'abc', 'def' ]
      file_path = 'dummy.yaml'
      expect(YAML).to receive(:dump).with(messages, anything)
      phrase.save_to_yaml(messages, file_path)
    end
  end
end
