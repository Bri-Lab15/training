...
describe NumberGame do
  ...
  # ASSIGNMENT #1: custom matcher for solution between 0 and 9
  matcher :be_between_zero_and_nine do
  ...
  # ASSIGNMENT #2: guess not equal to solution
  context 'when user guess is not correct' do
    subject(:wrong_guess_game) { described_class.new(7, '3') }

    it 'is not game over' do
      expect(wrong_guess_game).not_to be_game_over
    end
  end

  # ASSIGNMENT #3: verify_input invalid input
  context 'when given invalid input as argument' do
    it 'returns nil' do
      expect(game.verify_input('a')).to be_nil
    end
  end

  # ASSIGNMENT #4: player_turn with two incorrect inputs then valid
  context 'when user inputs two incorrect values, then a valid input' do
    subject(:game_loop_two) { described_class.new }

    before do
      allow(game_loop_two).to receive(:player_input).and_return('x', 'y', '5')
    end

    it 'completes loop and displays error message twice' do
      expect(game_loop_two).to receive(:puts).with('Input error!').twice
      game_loop_two.player_turn
    end
  end

  # ASSIGNMENT #5: final_message when count is 2-3
  context 'when count is 2-3' do
  ...
  # ASSIGNMENT #6: final_message when count is 4 and over
  context 'when count is 4 and over' do
    subject(:game_four) { described_class.new(5, '5', 5) }

    it 'outputs correct phrase' do
      fail_phrase = "Sorry! Too many guesses. The random number was 5.\n"
      expect { game_four.final_message }.to output(fail_phrase).to_stdout
    end
  end
end
